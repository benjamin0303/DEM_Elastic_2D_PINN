...
class ExperimentUtil:
    def __init__(self, configuration_file) -> None:
        ...
        self.gateway_url = "https://ai.cybershuttle.org"
    ...
    def upload_input_file(self, f: Path) -> str:
        ...
        resp = requests.post(
            "https://ai.cybershuttle.org/api/upload", headers=headers, files=files
    ...
    def launch_experiment(self, experiment_id):
        ...
        headers = {"Authorization": f"Bearer {access_token}"}
        resp = requests.post(
            f"https://ai.cybershuttle.org/api/experiments/{quote(experiment_id)}/launch/",
    ...
    def get_all_files_for_experiment(self, experiment_id):
            ...
            headers = {"Authorization": f"Bearer {access_token}"}
            resp = requests.get(
                f"https://ai.cybershuttle.org/api/experiment-storage/{quote(experiment_id)}/",
    ...
    def launch_HyperOpt(
        ...
        computation_resource_name=None, queue_name=None, node_count=None, cpu_count=None,
        walltime=None, auto_schedule=False,
        **kwargs,
    ):
        ...
        input_mapping = {}
        # input_mapping["Optional_Data"] = self.get_input_files_by_pattern(file_uploads, glob="*.xlsx")

        type_mapping = [
            "STRING", "INTEGER", "FLOAT", "URI", "URI_COLLECTION"
        ]
      
        for input in inputs:
            if input.name in input_mapping:
                value = input_mapping.get(input.name)
            elif input.name in kwargs:
                value = kwargs.get(input.name)
            else:
                print(f"using default value for {input.name}: {input.value}")
                continue
            if value is not None:
                if input.type == 0:
                    input.value = str(value)
                elif input.type == 1:
                    # input.value = bytes(value) # int - this fails
                    pass
                elif input.type == 2:
                    # input.value = bytes(value) # float - this fails
                    pass
                elif input.type == 3:
                    input.value = str(value)
                elif input.type == 4:
                    ...
                    raise ValueError()
                print(f"assigned specified value for {input.name} ({type_mapping[input.type]}): {input.value}")
        ...
        if success:
            ...
            logger.warn(f"Launch failed {experiment_id}")
